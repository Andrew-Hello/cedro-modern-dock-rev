...
>
> **Rev v1.0.0** freezes the first fully tested enhanced baseline while keeping the original GPL-3.0 license and upstream attribution.
...
Cedro Modern Dock Rev keeps the lightweight launcher workflow of the upstream project, while adding stronger Windows desktop integration, taskbar-like running-app behavior, edge docking, application identity support, and portable configuration backup/restore.

## Rev v1.0.0 highlights
...
2. Download `CedroModernDock-Rev-v1.0.0-win-x64.zip` (or a newer Rev release).
...
For migration to another PC, exporting/importing the JSON is recommended. Program shortcuts still depend on their target application being installed at a compatible path. Packaged Windows apps and supported PWA entries use their Windows application identity when available.
...
- `CedroModernDock.Infrastructure.Windows` — Win32/DWM/Shell integration, icon extraction, application identity, JSON persistence, registry auto-start and tray behavior.
...
## Stable baseline and development branch

- `cedro-enhanced-window` — active Rev development branch.
- `stable/rev-v1.0.0` — frozen first stable Rev baseline once released.
- Release tag: `rev-v1.0.0`.

The stable baseline is intentionally kept separate so experimental future changes can be developed without losing a known-good build.
